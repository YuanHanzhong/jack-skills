...
export function scanAdsForCompleted(searchResults: any[]): ScanResult {
  ...
  const doneStatuses = [STATUS_DONE, STATUS_ABANDONED, STATUS_ARCHIVED, STATUS_FAILED];
...
export function classifyTargetDb(
  ...
  // 默认移到DWS
  console.warn(
    `[classifyTargetDb] No keyword match for title="${pageTitle}" category="${pageCategory}", falling back to DWS`
  );
  return "DWS";
}
...
export function buildPropertyMapping(
  ...
  if (Object.keys(sourceSchema).length === 0) {
    console.warn("[buildPropertyMapping] sourceSchema is empty, returning empty mapping");
    return {};
  }
  if (Object.keys(targetSchema).length === 0) {
    console.warn("[buildPropertyMapping] targetSchema is empty, returning empty mapping");
    return {};
  }
