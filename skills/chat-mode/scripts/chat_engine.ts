...
/** Emotional words that signal the user needs support — triggers chat mode entry alongside ENTER_TRIGGERS. */
const EMOTION_KEYWORDS = [
  "好累", "烦死了", "脑子停不下来", "焦虑", "迷茫", "崩溃",
  "自责", "纠结", "犹豫", "烦", "累", "丧", "废",
];

/** Phrases that signal the user wants to leave chat mode and return to task execution. */
const EXIT_TRIGGERS = [
  "回任务", "不聊了", "直接给答案", "执行模式", "帮我做", "别聊了",
];

/** Phrases that signal the user has reached cognitive clarity and is ready to close the deep conversation. */
 ...
 * TODO: Replace with real semantic generation using LLM; this scaffold uses
 *       the full `context` string as the topic anchor for each option.
 */
function generateOptions(context: string): string[] {
  return [
    `1\uFE0F\u20E3 关于「${context}」，最卡你的是什么`,
    `2\uFE0F\u20E3 你是什么时候开始有这个感觉的`,
...
function buildGoldenSentence(oldBelief: string, newInsight: string): string {
  if (!oldBelief.trim()) throw new Error("buildGoldenSentence: oldBelief must not be empty");
  if (!newInsight.trim()) throw new Error("buildGoldenSentence: newInsight must not be empty");
