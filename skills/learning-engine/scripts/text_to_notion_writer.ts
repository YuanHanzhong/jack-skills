...
import {
  STATUS_MAP, STATUS_IN_PROGRESS,
  MASTERY_UNTOUCHED, MASTERY_FAMILIAR, MASTERY_UNDERSTOOD, MASTERY_MASTERED,
} from "../_shared/constants.ts";
import { resolve, buildProps } from "../_shared/schema_resolver.ts";

// Map single-emoji flashcard status (DWD/three-pass) → ADS mastery label
function flashcardStatusToMastery(status: string): string {
  switch (status) {
    case "🔵": return MASTERY_FAMILIAR;      // 了解中
    case "🟡": return MASTERY_UNDERSTOOD;    // 理解中
    case "🟢": return MASTERY_MASTERED;      // 已掌握
    default:   return MASTERY_UNTOUCHED;     // ⬜ 未接触 / 🔴 需复习 → 未学习 (new record)
  }
}
import {
  generatePanoramicMarkdown,
  generateSystemSectionMarkdown,
  type IntentMap,
  type AtomicKnowledgePoint,
  type LearningArchiveState,
} from "./three_pass_scanner.ts";

// ── Constants ────────────────────────────────────────────────────────────────

/** Score threshold below which a point is considered "learning" (not untouched). */
const SCORE_THRESHOLD_LEARNING = 50;
/** Score threshold at or above which a point is considered "mastered". */
const SCORE_THRESHOLD_MASTERED = 85;
/** Score threshold at or above which a point is considered "internalized". */
const SCORE_THRESHOLD_INTERNALIZED = 95;
...
export class DwdWriter {
  ...
  static _scoreToStatus(score: number): string {
    ...
    if (score < SCORE_THRESHOLD_LEARNING) return STATUS_MAP.learning;
    if (score < SCORE_THRESHOLD_MASTERED) return STATUS_MAP.tested;
    if (score < SCORE_THRESHOLD_INTERNALIZED) return STATUS_MAP.mastered;
    return STATUS_MAP.internalized;
  }

  /**
   * Build the intent layer (panoramic map) for the DWD page top section.
   */
  static buildIntentLayer(
    ...
    return generatePanoramicMarkdown(materialTitle, intentMap, atomicPoints, env, createdAt);
  }

  /**
   * Build the folded system section for the DWD page (auto-maintained JSON blocks).
   */
  static buildSystemSection(
    state: LearningArchiveState,
    intentMap: IntentMap,
  ): string {
    return generateSystemSectionMarkdown(state, intentMap);
  }

  /**
   * Build ADS record properties for a single atomic knowledge point.
   * Call once per AtomicKnowledgePoint when batch-creating ADS records.
   *
   * Key mapping notes:
   *   - flashcard `status` (⬜/🔵/🟡/🟢/🔴) → ADS `mastery` field (掌握度)
   *   - ADS `status` (任务状态) is fixed to STATUS_IN_PROGRESS for new records
   *   - Extra fields packed into `note`: "intentRank|intentRole|level|readCount|testCount"
   */
  static buildAtomicPointAdsProps(opts: {
    title: string;
    flashcardStatus: string;   // ⬜/🔵/🟡/🟢/🔴 from AtomicKnowledgePoint.status
    masteryScore: number;
    ...
    const props: Record<string, unknown> = {
      title: opts.title,
      status: STATUS_IN_PROGRESS,          // ADS task status: 🔵 进行中 (fixed for new records)
      mastery: flashcardStatusToMastery(opts.flashcardStatus), // 掌握度 select
      masteryScore: opts.masteryScore,
      streakCorrect: opts.consecutiveCorrect,
      chapter: opts.sourceChapter,
      note: `${opts.intentRank}|${opts.intentRole}|${opts.level}|${opts.readCount}|${opts.testCount}`,
      ...(opts.lastTested ? { lastTestDate: opts.lastTested } : {}),
      ...(opts.dwdPageId ? { parentRel: opts.dwdPageId } : {}),
    };
    return buildProps("ADS", props);
  }
...
export class AdsWriter {
  static buildProperties(
    ...
    return {
      [resolve("ADS", "title")]: title,
      [resolve("ADS", "dwd_url")]: dwdUrl,
      [resolve("ADS", "ods_url")]: odsUrl,
      [resolve("ADS", "status")]: status,
      [resolve("ADS", "material_type")]: materialType,
    };
  }
