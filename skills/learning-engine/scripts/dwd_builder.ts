...
/** Maximum heading level rendered as a Markdown heading (deeper nodes use bold list items). */
const MAX_HEADING_DEPTH = 3;
/** Maximum recursion depth passed to _renderTree to prevent excessively deep nesting. */
...
export function buildDwdContent(
  ...
  const mastered = allLeaves.filter((kp) => kp.status === "mastered" || kp.status === "internalized").length;
...
export function _renderTree(
  ...
  for (const node of nodes) {
    if (!node.children.length) {
      ...
      lines.push("");
      lines.push(..._renderTree(node.children, Math.min(depth + 1, MAX_TREE_DEPTH)));
      lines.push("");
    }
  }
  return lines;
