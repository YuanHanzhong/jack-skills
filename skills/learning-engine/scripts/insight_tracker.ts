...
/** Minimum score required in isMastered() check. */
const MASTERY_MIN_SCORE = 75.0;

/** Minimum number of angles to select when count is unconstrained. */
const MIN_ANGLE_COUNT = 3;
/** Maximum number of angles to select. */
const MAX_ANGLE_COUNT = 5;
/** IDs of angles that are always included first in angle selection. */
const ANCHOR_ANGLE_IDS: ReadonlySet<string> = new Set(["scenario", "one_liner"]);
...
export function selectMasteryAngles(
  _insight: string,
  count = MIN_ANGLE_COUNT,
): MasteryAngle[] {
  const n = Math.max(MIN_ANGLE_COUNT, Math.min(MAX_ANGLE_COUNT, count));
  const anchors = MASTERY_ANGLES.filter((a) => ANCHOR_ANGLE_IDS.has(a.id));
  const others = MASTERY_ANGLES.filter((a) => !ANCHOR_ANGLE_IDS.has(a.id));
...
export class MasteryState {
  ...
  recordAngle(angleId: string, passed: boolean, score: number): void {
    ...
    if (passed) {
      ...
      // Remove from failed if previously failed but now passed
      const failIdx = this.anglesFailed.indexOf(angleId);
      if (failIdx !== -1) this.anglesFailed.splice(failIdx, 1);
    } else {
  ...
  isMastered(): boolean {
    return (
      ...
      this.currentScore >= MASTERY_MIN_SCORE
    );
  }
