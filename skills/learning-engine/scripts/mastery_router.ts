...
export class LayeredKnowledgePoint {
  ...
  get statusEmoji(): string {
    const base = this.stopThreshold;
    let key: string;
    if (this.score === 0) key = "untouched";
    else if (this.score < base) key = "learning";
    else if (this.score < base + 5) key = "tested";
    else if (this.score < base + 15) key = "mastered";
    else key = "internalized";
