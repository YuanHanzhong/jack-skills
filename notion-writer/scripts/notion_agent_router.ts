...
export class ReaderAgent {
  dedupCheck(adsId: string = NOTION_IDS.ads): Record<string, unknown> {
    return {
      action: "notion-fetch",
      id: adsId,
      purpose: "dedup_check",
  ...
  run(task: AgentTask): AgentTask {
    ...
    try {
      if (task.intent === "查重" || task.intent === "dedup_check") {
        task.result = this.dedupCheck((task.payload.ads_id as string) ?? NOTION_IDS.ads);
