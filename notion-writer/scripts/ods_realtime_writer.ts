...
function getTsDisplay(): string {
  const now = new Date().toLocaleString("sv-SE", { timeZone: "Asia/Shanghai" });
  return now.slice(0, 16);
}

function getTsIso(): string {
  const now = new Date().toLocaleString("sv-SE", { timeZone: "Asia/Shanghai" });
  // "2026-03-11 14:30" → "2026-03-11T14:30:00+08:00"
...
export async function writeToLocalBuffer(row: OdsRow): Promise<Record<string, unknown>> {
  ...
  const dir = join(ODS_BUFFER_PATH, "..");
  await Bun.write(join(dir, ".keep"), "");
  const existing = await Bun.file(ODS_BUFFER_PATH).text().catch((e: unknown) => { console.warn("[ods_realtime_writer] buffer read failed:", e); return ""; });
  await Bun.write(ODS_BUFFER_PATH, existing + JSON.stringify(record) + "\n");
