...
/**
 * Log database row title format.
 */
export function buildLogDbTitle(ts: string, purpose: string): string {
  return `${ts} · ${purpose}`;
}

/**
 * DWS document title format.
 */
export function buildDwsTitle(ts: string, category: string, purpose: string): string {
  return `${ts}-DWS-${category}-${purpose}`;
}
