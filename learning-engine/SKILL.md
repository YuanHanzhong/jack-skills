...
| L3 场景活用 | 遇到能不能做出来 | 3题(真实高压情境) | 角色扮演、决策模拟、冲突应对 |

### L3 出题 + 纠正 + 迭代话术流程 [CRITICAL]
...
// 第一步：出题——三角度出三个真实高压场景
FOR each 知识点:
  出题_角度1 = "遇到了[高压情境A]，你会怎么做？"
  出题_角度2 = "遇到了[高压情境B]，你第一反应是什么？"
  出题_角度3 = "遇到了[高压情境C]，你会怎么回应？"
...
收到用户回答后:
  分析用户回答的薄弱点
  给出本次【X%】
  指出不准确/不完整的地方（不是全盘否定，而是指出可以更好的点）
  给出该知识点澄清后的结论

// 第三步：迭代出适配用户风格的专属话术
基于用户回答风格:
  把澄清结论迭代成用户说出来顺口、有力、可直接用的专属话术
  让用户确认或进一步打磨
  最终产出可写入 ADS 的「专属话术 + 使用场景」

// 循环条件
直到用户确认该话术已顺口有力，再换下一个知识点
```
