...
export function calculateNewScore(
  ...
  if (answerResult === "wrong") {
    return [Math.max(0.0, currentScore - 10.0), 0];
  }
  if (answerResult === "hinted") {
    return [Math.max(0.0, currentScore - 25.0), 0];
  }
  if (answerResult === "partial") {
    return [Math.min(currentScore + 5.0, 74.0), consecutiveCorrect];
  }
  // correct
  const n = consecutiveCorrect + 1;
  let score: number;
  if (n === 1) score = Math.min(Math.max(currentScore, 60.0) + 10.0, 75.0);
  else if (n === 2) score = Math.min(currentScore + 12.0, 85.0);
  else if (n === 3) score = Math.min(currentScore + 8.0, 92.0);
  else if (n === 4) score = Math.min(currentScore + 5.0, 94.0);
  else score = 95.0;
  return [score, n];
}
...
export function buildScoreFeedback(
  ...
  if (answerResult === "correct" && consecutive >= 3) {
